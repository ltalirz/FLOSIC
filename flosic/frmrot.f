        ...
        open(300,file='FRMIDT')
             ...
             if(max(nidt(1),nidt(2)).gt.mxidt) then
               print*,'mxidt too small in symfrm, must be atleast '
     &         ,nidt(1)+nidt(2)
               stop
             end if
             do ispn=1,2   
             ...
             read(300,50)line
 50          format(a100)
             read(line,*)(fodidt(j,idnt,ispn),j=1,3)
             print  100,(fodidt(j,idnt,ispn),j=1,3)
             end do
             end do
        close(300)
